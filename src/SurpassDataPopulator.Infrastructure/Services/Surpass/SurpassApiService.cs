...
public class SurpassApiService : ISurpassApiService
{
    ...
    {
        //TODO what happens with no subjects - can i check for specific errors in this list instead related to auth?
        var result = await _surpassV2Api.SubjectTop1(); 
        if (result.Errors != null && result.Errors.Any()) throw new Exception($"Error(s) while testing the connection : {result.Errors.Humanize()}");
    }
    ...
    public async Task AddMediaToSubject(List<Item> items, string subjectReference)
    {
        //if any item has media, then we need to make sure this is added to the subject.
        var allImages = items.SelectMany(i => i.MediaImages).DistinctBy(m => m.Name).ToList();

        if (!allImages.Any()) return;

        var imageIdLookup = new Dictionary<long?, MediaImage>();

        //TODO Support running this against the same subject multiple times - this will currently re upload the same media on each run
        foreach (var mediaImage in allImages)
        {
            var response = await _surpassV2Api.Media(new MediaInputResource
            {
                Subject = new ItemSubjectResource
                {
                    Reference = subjectReference
                },
                Name = mediaImage.FileName,
                Data = mediaImage.Image,
            });

            if (response.Errors != null && response.Errors.Any()) throw new Exception($"Error(s) while creating media item {mediaImage.Name}: {response.Errors.Humanize()}");
            if (response.Errors != null && response.Errors.Any()) throw new Exception($"Error(s) while creating media item {mediaImage.Name}: {response.Errors.Humanize()}");
            if (response.Id == null) throw new Exception($"Error(s) while creating the media item. Response Id was null");

            imageIdLookup.Add(response.Id, mediaImage);
        }

        //now assign the id of the uploaded media to the items before we can post to surpass
        foreach (var item in items)
        {
            foreach (var mediaImage in item.MediaImages)
            {
                var image = imageIdLookup.FirstOrDefault(x => x.Value.Name == mediaImage.Name);
                if (image.Value == null) throw new Exception($"Cannot find image {mediaImage.Name} in the image lookup");
                mediaImage.Id = image.Key.Value;
            }
        }
    }
    ...
    private async Task AssignTagValuesToItems(Dictionary<long, Item> items, Dictionary<string, long> tagGroupLookup, Dictionary<string, long> tagValueLookup, string subjectReference)
    {
        foreach (var (key, value) in items)
        {
            ...
            foreach (var tag in value.Tags)    
            {
                ...
                //TODO Check if it already exists before creating a new one, and assign the existing one.
    ...
    {
        await AddMediaToSubject(items, subjectReference);
    ...
    private async Task<Dictionary<string, long>> CreateTagGroupsForItems(List<Item> items, string subjectReference)
    {
        ...
        foreach (var distinctTagGroup in distinctTagGroups)
        {
            ...
            if (response.Count >= 1)
            {
                //The check for existing tag groups might find multiple since we have to use a contains search function using the name we want.. It might return multiple
                //So we need to find the exact match from the returned tag groups
                var tag = response.Response.FirstOrDefault(x => x.Name == distinctTagGroup.Name);
                exists = tag != null;

                //if we find a match then we have an existing tag group to use, otherwise we still need to create it
