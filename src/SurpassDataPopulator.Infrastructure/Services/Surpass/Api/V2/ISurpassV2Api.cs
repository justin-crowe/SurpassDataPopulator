...
{
    [Get("/Subject?$top=1")]
    Task<TimeZonePageResponse<SubjectDetailedResource>> SubjectTop1();

    [Get("/Subject?reference={reference}")]
    Task<TimeZonePageResponse<SubjectDetailedResource>> SubjectSearch(string reference);

    [Post("/Subject")]
    Task<TimeZonePostResponseModel> Subject([Body] SubjectCreateResource subjectCreateResource);
    ...
    //Media
    [Post("/Media")]
    Task<PostResponseModel> Media([Body] MediaInputResource media);

    [Get("/Media/{id}")]
    Task<TimeZonePageResponse<MediaDetailedResource>> MediaSearch(string id);
}